...
fetch('https://sheets.googleapis.com/v4/spreadsheets/1zaCd8es0pm2md9X8AJwJRm-0J22_7UxoRNAztGzpkmk/values/Partidos!G2:N?key=AIzaSyD1qXjPmgBaRtX0zJtH76nvU708Gvs3A-g')
.then(response => response.json())
  .then(data => {
    ...
    function compararFechas(partidoA, partidoB) {
      ...
      } else if (fechaA === 'undefinedTundefined') {
        return 1; // Partido A está en blanco, se coloca después de partido B
      } else if (fechaB === 'undefinedTundefined') {
    ...
    jornadas.forEach((jornada, index) => {
      container.innerHTML += `<h1>Jornada ${index + 1}</h1>`;
